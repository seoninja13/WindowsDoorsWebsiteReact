 ...
 * Simple Crawl
 *
 * This script runs a simplified crawl of the Window World LA website,
 * extracting all URLs, content, screenshots, UI components, images, and other assets.
 * It uses a simple file-based logging system instead of Supabase.
 */
...
function log(level, message) {
  ...
  fs.appendFile(
    path.join(__dirname, '..', 'crawl_results', 'crawl.log'),
...
async function fetchUrl(url) {
  try {
    ...
    log('INFO', `Fetched ${url} in ${durationMs}ms`);
...
function extractLinks(html, baseUrl) {
  try {
    ...
    log('ERROR', `Error extracting links: ${error.message}`);
    return [];
  }
}
...
function extractImages(html, baseUrl) {
  try {
    ...
    log('ERROR', `Error extracting images: ${error.message}`);
    return [];
  }
}
...
function extractComponents(html) {
  try {
    ...
    log('ERROR', `Error extracting components: ${error.message}`);
    return [];
  }
}
...
async function takeScreenshot(url, outputPath) {
  try {
    ...
    log('INFO', `Took screenshot of ${url} in ${durationMs}ms`);

    return true;
  } catch (error) {
    log('ERROR', `Error taking screenshot of ${url}: ${error.message}`);
    return false;
  }
}
...
async function saveStats() {
  try {
    ...
    log('INFO', `Crawl statistics saved to ${path.join(OUTPUT_DIR, 'crawl_stats.json')}`);
  } catch (error) {
    log('ERROR', `Error saving crawl statistics: ${error.message}`);
  }
}
...
async function saveUrls() {
  try {
    ...
    log('INFO', `URLs saved to ${URLS_FILE}`);
  } catch (error) {
    log('ERROR', `Error saving URLs: ${error.message}`);
  }
}
...
async function generateSitemap() {
  try {
    ...
    log('INFO', `Sitemap generated and saved to ${SITEMAP_FILE}`);
  } catch (error) {
    log('ERROR', `Error generating sitemap: ${error.message}`);
  }
}
...
async function main() {
  try {
    log('INFO', `Starting crawl of ${BASE_URL}`);
    ...
    while (urlQueue.length > 0 && stats.pagesCrawled < MAX_PAGES) {
      ...
      // Log progress
      log('INFO', `Progress: ${stats.pagesCrawled}/${MAX_PAGES} pages crawled, ${urlQueue.length} URLs in queue`);
    }
    ...
    log('INFO', `Crawl completed. Processed ${stats.pagesCrawled} pages.`);
  } catch (error) {
